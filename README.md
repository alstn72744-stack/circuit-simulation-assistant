...
사용자가 작성한 LTspice 회로를 검토·승인한 조건으로 실행하고, RAW 데이터를 Python으로 측정·비교하는 Windows / Streamlit 프로젝트입니다. 자연어 조건은 현재 rule-based parser로 처리합니다.

## Why I Built It

전자회로 실험에서 directive 문법을 반복해서 확인하고, waveform 수치를 수동 측정하고, 소자 값을 바꾸며 simulation·결과 비교·report 정리를 반복하는 시간을 줄이고 싶었습니다.

처음에는 arbitrary circuit generation도 고려했습니다. 그러나 current mirror와 feedback 같은 복잡한 topology를 일반화하기보다 **v1에서는 schematic은 사용자가 작성하고 simulation / analysis automation에 집중**하도록 범위를 줄였습니다. [문제 정의](problem-definition.md)와 [SPEC](SPEC.md)은 이 판단과 장기 계획을 담습니다. SPEC의 목표 기능이 모두 구현된 것은 아닙니다.
...
- **R/C Parameter Sweep:** 한 소자의 명시적 값 목록 또는 start/stop/step, AC/Transient/DC 반복 실행, 비교 표·metric graph·overlay, 실패 point 기록과 다음 point 실행.
- **Analysis Summary:** measured / derived facts, warnings, comparison, evidence를 JSON-compatible 구조로 분리.
- **AI architecture:** LLM-ready prompt preview/export, OpenAI/mock provider, numerical consistency validation. 수치·단위 일치 검사는 물리적 원인이나 해석의 정확성을 증명하지 않습니다.
...
- 외부 model/include 업로드, 결과 persistence/reanalysis, 긴 legend·좁은 화면 UX는 후속 과제입니다.
- Image-to-circuit와 report generation은 future work입니다.
